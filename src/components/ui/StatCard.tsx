...
import { KPITooltip } from "../shared/KPITooltip";
  ...
  return (
    <Card className={`p-6 rounded-xl shadow-sm ${color ? colorClasses : variantClasses[variant]}`}>
      <div className="space-y-3">
        ...
        <div className={`text-4xl md:text-5xl font-bold tracking-tight ${numberColorClasses || "accent-heading"}`}>
          {displayValue}
        </div>
        {displaySubtext && <p className="text-sm text-black/60 dark:text-white/60">{displaySubtext}</p>}
        {optionalCTA && (
          <Button
